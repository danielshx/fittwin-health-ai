...
import { AgentRecommendation, DailyPlan } from "@/types";
import { computeReadiness, computeBurnoutRisk } from "@/lib/metrics";

export class PlannerAgent implements AIAgent {
  id = "planner";
  name = "PlannerAgent";
  description = "Generates daily plans and prioritizes tasks based on health metrics";
  ...
  analyze(context: AgentContext): AgentRecommendation[] {
    ...
    const plan = this.generateDailyPlan(context);

    // Daily priorities recommendation
    recommendations.push({
      ...
      type: "training",
      title: "Today's Plan",
      rationale: `Based on your readiness (${readiness.score}/100) and recent trends, here are your top priorities.`,
      priority: "high",
      actions: [
  ...
  private generateDailyPlan(context: AgentContext): DailyPlan {
    const { profile, today, last7Days, baseline } = context;
    const readiness = computeReadiness(today, baseline, last7Days);
    const burnout = computeBurnoutRisk(last7Days, baseline);
